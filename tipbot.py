...
def main():
    ...
    for curr_dm in dm_list:
        ...
        if curr_dm.sender_id != 966739513195335680:
    ...
    get_unprocessed = ("SELECT dm_id, sender_id, dm_text FROM dm_list WHERE processed=0 AND sender_id <> {}".format(BOT_ID))
    unprocessed_dms = getDbInfo(get_unprocessed)
    for row in unprocessed_dms:
        # Initialize tip error flag
        tip_error = 0
        dm = setDm(row[0], row[1], row[2])
        ...
        if dm_array[0].lower() == '!help':
            help_message = ("Thank you for using the Nano Tip Bot!  Below is a list of commands, and a description of " 
                            "what they do:\n\n" + BULLET.encode("utf-8") + " !help: That's this command! Shows a list of " 
                            "all available commands and what they do.\n" + BULLET.encode("utf-8") + " !register: " 
                            "Registers your twitter ID for an account that's tied to it.  This is used to store your"
                            " tips. Make sure to withdraw to a private wallet, as the tip bot is not meant to be a " 
                            "long term storage device for Nano.\n" + BULLET.encode("utf-8") + " !balance: This" 
                            " returns the balance of the account linked with your Twitter ID.\n" +
                            ...
                            BULLET.encode("utf-8") + " !account: Once your registered, send !account to see your " 
                            "account.  You can use this to deposit more Nano to tip from your personal wallet.\n" +
                            BULLET.encode("utf-8") + " !withdraw: Proper usage is !withdraw xrb_12345.  This will send " 
                            "the full balance of your tip account to the provided Nano account.\n" +
                            BULLET.encode("utf-8") + " !donate: Proper usage is !donate 1234.  This will send the " 
                            "requested donation to the Nano Tip Bot donation account to help fund the developer's " 
                            "efforts.\n\nNOTE: The tipbot processes actions at the top of every minute.  Please be" 
                            " patient!")
                        ...
                        if not data:
                            ...
                            if wrong_tip == 1:
                                ...
                                low_balance_text = ("Your balance is only {} NANO and you tried to send {}.  Please add "
                                                    "more NANO to your account, or lower your tip amount.".format(balance,
                                                                                                        float(dm_array[2])))
                                tip_error = 1
                                ...
                                if not receiver_account_data:
                                    ...
                                    create_receiver_account = ("INSERT INTO users\
                                                    (user_id, account, register) VALUES({}, '{}',0)".format(receiver_id,
                                                                                                            receiver_account))
                                    setDbInfo(create_receiver_account)
                                    print("{}: Sender sent to a new receiving account.  Created  account {}".format(str(datetime.now()),
                                                                                                            receiver_account))
                                else:
                                ...
                                sent_tip_text = ("You've successfully sent a {} NANO tip to @{}!  You can check the transaction" 
                                                "at https://www.nanode.co/block/{}".format(dm_array[2], receiver_id_info.screen_name, send_hash))
            ...
            if len(dm_array) == 2:
                ...
                data = getDbInfo(withdraw_account_call)
                if not data:
            ...
            if len(dm_array) == 2:
                ...
                data = getDbInfo(sender_account_call)
                sender_account = data[0][0]
                ...
                if wrong_donate == 1:
                    ...
                    print("{}: {} NANO donation processed.  Hash: {}".format(str(datetime.now()), float(dm_array[1]), send_hash))
            else:
                tip_error = 1
        ...
        if tip_error == 1:
            ...
            rate_limit_fix = ("UPDATE dm_list SET processed=4, dm_response='{}' WHERE dm_id={}".format(str(stored_response), dm.id))
            setDbInfo(rate_limit_fix)
            print("{}: Rate limit hit.  DM set to reprocess.".format(str(datetime.now())))
        else:
            processed_set = ("UPDATE dm_list SET processed=2 WHERE dm_id={}".format(dm.id))
