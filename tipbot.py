...
for row in unprocessed_dms:
	...
	if dm_array[0].lower() == '!help':
		api.send_direct_message(user_id = dm.sender_id, text = "Thank you for using the Nano Tip Bot!  Below is a list of commands\
			, and a description of what they do:\n\n" + BULLET.encode("utf-8") + 
			" !help: That's this command!  Shows a list of all available commands and what they do.\n" + BULLET.encode("utf-8") +
			" !register: Registers your twitter ID for an account that's tied to it.  This is used to store your tips. \
			Make sure to withdraw to a private wallet, as the tip bot is not meant to be a long term storage device for Nano.\n" + BULLET.encode("utf-8") +
			" !balance: This returns the balance of the account linked with your Twitter ID.\n" + BULLET.encode("utf-8") +
			" !tip: Proper usage is !tip <@username> <amount of tip>.  This will send the requested amount of Nano to the account \
			linked to that user's twitter ID.\n" + BULLET.encode("utf-8") +
			" !account: Once your registered, send !account to see your account.  You can use this to deposit more Nano to tip from \
			your personal wallet.\n" + BULLET.encode("utf-8") +
			" !withdraw: Proper usage is !withdraw <xrb_12345>.  This will send the full balance of your tip account to the provided Nano account.\
   			\n" + BULLET.encode("utf-8") + " !donate: Proper usage is !donate <amount of donation>.  This will send the requested donation to the Nano Tip Bot donation \
			account to help fund the developer's efforts.\n\nNOTE: The tipbot processes actions at the top of every minute.  Please be patient!")
		...
		if data == None:
			...
			api.send_direct_message(user_id = dm.sender_id, text = "There is no account linked to your username.  Please respond with !register to create an account!")
		else:
			...
			db.commit()
			api.send_direct_message(user_id = dm.sender_id, text = "You have successfully registered for an account.  Your account is {}.".format(sender_account))
			...
			api.send_direct_message(user_id = dm.sender_id, text = "You're already registered.  Your account number is {}".format(sender_account))
		...
		if len(dm_array) == 3:
			...
			if float(receiver_id_info.id) == float(BOT_ID):
				...
				api.send_direct_message(user_id = dm.sender_id, text = "You can't tip the bot, silly!  If you'd like to donate, try using the !donate command.")
				...
				if data == None:
					...
					if wrong_tip == 1:
						...
						api.send_direct_message(user_id = dm.sender_id, text = "Your balance is only {} Nano and you tried to send {}. \
									Please add more Nano to your account, or lower your tip amount.".format(balance, float(dm_array[2])))
					elif float(dm_array[2]) < 0.00001:
						print("Sender tried to send less than 0.00001")
						api.send_direct_message(user_id = dm.sender_id, text = "The minimum tip amount is 0.00001.  Please update your tip amount and resend.")
					else:
						#retrieve the receiver's account from the db
						cursor.execute("SELECT account FROM users\
							where user_id = {}".format(receiver_id))
						receiver_account_data = cursor.fetchone()
						if receiver_account_data == None:
							#Create an account number and insert into the db
							receiver_account = rpc.account_create(wallet="{}".format(WALLET), work=True)
							cursor.execute("INSERT INTO users\
											(user_id, account, register) VALUES({}, '{}',0)".format(receiver_id, receiver_account))
							db.commit()
						...
						api.send_direct_message(user_id = dm.sender_id, text = "You've successfully sent a {} NANO tip to @{}! \
							You can check block hash {} on https://www.nanode.co/".format(dm_array[2], receiver_id_info.screen_name, send_hash))
						#Inform the receiver that they got a tip!
						dm_send_amount = float(dm_array[2])
						sender_id_info = api.get_user(dm.sender_id)
						api.update_status(status="Hey @{}, @{} just sent you a {} $NANO tip!  Send @nanotipbot a DM with !register to claim your funds or !help \
											for more commands.  If you want to learn more about Nano, go to https://nano.org/".format(receiver_id_info.screen_name,sender_id_info.screen_name, dm_array[2])) 
		else:
			api.send_direct_message(user_id = dm.sender_id, text = "Incorrect synatx.  Please use the format !tip @username 123")
		...
		if len(dm_array) == 2:
			...
			data = cursor.fetchone()
			if data == None:
				api.send_direct_message(user_id = dm.sender_id, text = "You do not have an account.  Respond with !register to set one up.")
				...
				if rpc.validate_account_number(receiver_account) == 0:
					...
					api.send_direct_message(user_id = dm.sender_id, text = "The account number you provided is invalid.  Please double check and resend your request.")
					...
					print("The user tried to withdraw with 0 balance")
                                	api.send_direct_message(user_id = dm.sender_id, text = "You have a 0 balance in your account.  Please deposit to your address \
                                                                                        {} to send more tips!".format(sender_account))
				else:
					...
					api.send_direct_message(user_id = dm.sender_id, text = "You've successfully withdrawn {} NANO to account {}.  \
						You can check block hash {} on https://www.nanode.co/".format(float(balance), receiver_account, send_hash))
					print("Withdraw processed.  Hash: {}".format(send_hash))
		else:
			print("User sent a withdraw with invalid syntax.")
			api.send_direct_message(user_id = dm.sender_id, text = "Incorrect synatx.  Please use the format !withdraw (account)") 
		...
		if len(dm_array) == 2:
			...
			#Convert the balance to Nano units
				...
				print("User tried to donate more than their balance.")
                        	api.send_direct_message(user_id = dm.sender_id, text = "Your balance is only {} Nano and you tried to send {}. \
                                	Please add more Nano to your account, or lower your donation amount.".format(balance, float(dm_array[1])))
			elif float(dm_array[1]) < 0.000001:
				print("User tried to donate less than 0.000001")
				api.send_direct_message(user_id = dm.sender_id, text = "The minimum donation amount is 0.000001.  Please update your donation amount and resend.")
			else:
				#convert the send amount to raw
				...
				api.send_direct_message(user_id = dm.sender_id, text = "Thank you for your generosity!\
                                                                                       You can check your donation status under block hash {} on https://www.nanode.co/".format(send_hash))
				print("Donation processed.  Hash: {}".format(send_hash))
                else:
			print("User sent a donation with invalid syntax")
                        api.send_direct_message(user_id = dm.sender_id, text = "Incorrect synatx.  Please use the format !donate (amount)")
		...
		cursor = db.cursor()
                ...
                data = cursor.fetchone()
		if data == None:
			api.send_direct_message(user_id = dm.sender_id, text = "You do not have an account.  Please respond with !register to set one up.")
		...
		api.send_direct_message(user_id = dm.sender_id, text = "That command or syntax is not recognized.  Please send !help for a list of commands and what they do.")
