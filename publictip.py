...
class MyStreamListener(tweepy.StreamListener):
    ...
    def tipProcess(self, dm_action, dm_array, dm_id, sender_screen_name, dm_text):
                    ...
                    if str(dm_array[index])[0] == "@":
                        try:
                            ...
                            if dm_array[index].lower() != 'nanotipbot' and dm_array[index].lower() != '@nanotipbot':
                                ...
                                logging.info("{}: User {} added to tip list.".format(datetime.now(TIMEZONE),dm_array[index]))
                            else:
                        ...
                        logging.info("{}: Not a user, ignored.".format(datetime.now(TIMEZONE)))
        ...
        logging.info("{}: Account information retreived.".format(datetime.now(TIMEZONE)))
        if not sender_account_info:
            ...
            logging.info("{}: account {} register {} retrieved.".format(datetime.now(TIMEZONE),sender_account, register))
            ...
            logging.info("{}: total tip amount: {}, balance: {}".format(datetime.now(TIMEZONE),total_tip_amount,balance))
